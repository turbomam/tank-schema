...
all: site
site: gen-project gendoc project/json/tank_schema.json
%.yaml: gen-project
...
gen-examples:
	cp src/data/examples/valid/TankCollection*.yaml $(EXAMPLEDIR)
...
examples/output: src/tank_schema/schema/tank_schema.yaml
	mkdir -p $@
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/tank_schema/schema/*yaml src/*/datamodel/*py src/data src/docs tests src/*/_version.py project/reports/ src/local_schemasheets/ data_harmonizer/
...
target/usage_template.tsv: src/tank_schema/schema/tank_schema.yaml
	mkdir -p $(dir $@)
...
project/json/tank_schema.json: src/tank_schema/schema/tank_schema.yaml
	mkdir -p $(@D)
	$(RUN) gen-linkml \
		--format json  \
